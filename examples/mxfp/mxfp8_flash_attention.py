...
def _mxfp8_attn_fwd_inner(
    ...
    for start_n in tl.range(lo, hi, BLOCK_N):
        ...
        # E8M0: encoding = ceil(log2(amax)) + 127, clamped to [0, 254]
        ...
        # Compute float scale for quantizing P: scale = 2^(e8m0 - 127)
        p_scale_f32 = tl.math.exp2((p_e8m0.to(tl.float32) - 127.0))
        # Expand scale to [BLOCK_M, BLOCK_N] for division
        p_scale_expanded = tl.reshape(
            tl.broadcast_to(p_scale_f32[:, :, None], [BLOCK_M, BLOCK_N // 32, 32]),
            [BLOCK_M, BLOCK_N]
        )
        # Quantize P: divide by scale then cast to fp8
