"""Run as proton target - simple kernel invocations for profiling."""
import torch
import sys
import os
...
# warmup
for _ in range(3):
    out = mxfp4_flash_attention(q_packed, k_packed, v_packed, q_scale, k_scale, v_scale, causal=False, sm_scale=sm_scale)
torch.cuda.synchronize()

proton.enter_scope("mxfp4_fa_B2H32N4096D128")
for _ in range(10):
    out = mxfp4_flash_attention(q_packed, k_packed, v_packed, q_scale, k_scale, v_scale, causal=False, sm_scale=sm_scale)
torch.cuda.synchronize()
proton.exit_scope()
